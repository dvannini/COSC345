...
Interface::Interface() { //THIS DOES NOT RUN
    ...
    if (!SetConsoleMode(hConsole, dwOutMode))
    {
        ...
        if (!SetConsoleMode(hConsole, dwOutMode))
        {
            ...
            std::cout << "failed to set virtual terminal mode.";
            return;
        }
    }
...
int Interface::_test() {
    ...
    /*try {
        ...
        a.addSound(2, sequence);
        a.addSound(3, sequence);
    ...
    /*try {
        ...
        a.performAction(49, testSequence);
        a.performAction(50, testSequence);
    }
    catch (std::exception& e) {
    ...
    try {
        Interface a;
        std::map<std::string, std::array<bool, 8>> testSequence;
        int result = a.performAction('z', testSequence);
        if (result != -1) {
